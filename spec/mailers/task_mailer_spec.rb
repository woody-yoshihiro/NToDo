...
describe TaskMailer, type: :mailer do
  ...
  describe "#creation_email" do
    ...
    it "規程どおりのメールが生成されている" do
      # ヘッダー
      expect(mail.subject).to eq("リスト作成完了メール")
      expect(mail.to).to eq(["user@example.com"])
      expect(mail.from).to eq(["NToDo@example.com"])

      # text形式の本文
      expect(text_body).to match("以下のリストを作成しました")
      expect(text_body).to match("メイラーSpecを書く")
      expect(text_body).to match("送信したメールの内容を確認します")

      # html形式の本文
      expect(html_body).to match("以下のリストを作成しました")
